function graph = load_g2o_file_pudq(g2o_file)
...
disp(['Done reading now computing odometry edges'])
...
for i = 1:num_vertices
    if i == 1 
        ...
        edge_index = find(edges(:, 1) == (i - 1) & edges(:, 2) == i, 1, 'first');

        if isempty(edge_index)
            error('No edge found between two vertices')
        end

        t_ij = t{edge_index};
        R_ij = R{edge_index};
        t_i = vertices{i-1}(1:2);
        theta_i = vertices{i-1}(3);
        R_i = [cos(theta_i), -sin(theta_i);
               sin(theta_i), cos(theta_i)];

        t_j = t_i + R_i * t_ij;
        R_j = R_i * R_ij;
        theta_j = theta_to_R(R_j);

        vertices{i} = [t_j', theta_j]';
        vertices_pudq{i} = pose_to_pudq(vertices{i});
    end
end
disp(['Done computing odometry edges'])
